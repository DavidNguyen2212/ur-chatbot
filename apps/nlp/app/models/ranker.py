...
class CoolChatVectorStore(PineconeVectorStore):
    ...
    def coolchat_similarity_search(
        ...
        results: List[Tuple[Document, float]] = (
            self.similarity_search_with_relevance_scores(query=query, k=k, **kwargs)
        )
        ...
        if max_prio == min_prio:
            ...
            scaled_priorities = [
                (x - min_prio) / (max_prio - min_prio) for x in priorities
            ]

        # Arrange!
        sorted_results = sorted(
            enumerate(results),
            key=lambda elem: scaled_priorities[elem[0]] * self.w_p
            + elem[1][1] * self.w_s,
            reverse=True,
        )
        ...
        unique_docs = {
            doc.metadata.get("id", id(doc)): doc for doc in sorted_results
        }.values()
    ...
    async def acoolchat_similarity_search(
        ...
        results: List[Tuple[Document, float]] = (
            await self.asimilarity_search_with_relevance_scores(query, k=k, **kwargs)
        )
...
class CoolChatRetriever(BaseRetriever):
    ...
    async def _aget_relevant_documents(self, query: str) -> List[Document]:
        return await self.vectorstore.acoolchat_similarity_search(
