...
class CoolChatVectorStore(PineconeVectorStore):
    ...
    def _deduplicate_docs(
        self, docs: List[Document], seen: Optional[Set[str]] = None
    ) -> List[Document]:
    ...
    async def _process_batch(
        ...
        sorted_docs = [
            doc for doc, _ in sorted(weighted_scores, key=lambda x: x[1], reverse=True)
        ]

        return self._deduplicate_docs(sorted_docs)

    async def _keyword_search(self, query: str, k: int = 3) -> List[SearchResult]:
    ...
    async def acoolchat_similarity_search_original(
        ...
        try:
            ...
            processed_results = await self._process_results_efficiently(
                vector_results, keyword_results, k=k
            )
    ...
    async def _process_results_efficiently(
        ...
        for doc, score in sorted(
            vector_results + keyword_results, key=lambda x: x[1], reverse=True
        )[:k]:
            doc_id = self._get_document_id(doc)
    ...
    async def batch_similarity_search(
        self, queries: List[str], k: int = 3, **kwargs
    ) -> List[List[Document]]:
        """Batch process multiple queries efficiently"""
        tasks = [
            self.acoolchat_similarity_search(query, k=k, **kwargs) for query in queries
        ]
    ...
    async def hybrid_search(
        ...
        **kwargs,
        ...
        try:
            ...
            final_results.sort(
                key=lambda x: (x.vector_score * alpha + x.keyword_score * (1 - alpha)),
                reverse=True,
            )
            ...
            filtered_results = [
                ...
                if (result.vector_score * alpha + result.keyword_score * (1 - alpha))
                >= self.min_similarity
            ]
...
class CoolChatRetriever(BaseRetriever):
    ...
    request_semaphore: ClassVar[asyncio.Semaphore] = asyncio.Semaphore(
        5
    )  # <-- annotate
    ...
    async def batch_get_relevant_documents(
        self, queries: list[str], batch_size: int = 5
    ) -> list[list[Document]]:
        results = []
