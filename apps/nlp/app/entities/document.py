import datetime
import uuid
from sqlalchemy import Column, String, Boolean, Integer, Enum, DateTime, Text, func
from sqlalchemy.dialects.postgresql import UUID
from app.entities import Base
from app.enums.document import DocumentTypeEnum, PriorityEnum, TrainingStatusEnum


class CoolDocument(Base):
    ...
    training_status = Column(
        Enum(TrainingStatusEnum), default=TrainingStatusEnum.UNTRAINED
    )
    ...
    updated_at = Column(
        DateTime(timezone=True),
        server_default=func.now(),
        onupdate=lambda: datetime.datetime.now(datetime.UTC) # should create a trigger in production, not python lambda. 
    )
