from typing import Any, ClassVar, Dict, List, Tuple, Optional, override
...
class CoolChatVectorStore(PineconeVectorStore):
    ...
    def _preprocess_text(self, text: str) -> str:
        ...
        text = re.sub(
            r"[^\w\sáàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệóòỏõọôốồổỗộơớờởỡợíìỉĩịúùủũụưứừửữựýỳỷỹỵđ]",
            " ",
            text,
        )
    ...
    async def hybrid_search(self, query: str, k: int = 3, **kwargs) -> List[Document]:
        # Thực hiện song song vector và keyword
        vector_task = asyncio.create_task(
            self.asimilarity_search_with_relevance_scores(query, k=k, **kwargs)
        )
        keyword_task = asyncio.create_task(self._keyword_search(query, k))
