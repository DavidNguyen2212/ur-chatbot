...
from app.integrations.pinecone.retriever import CoolChatVectorStore
...
def get_embedder():
    return OpenAIEmbeddings(
        model="text-embedding-3-small", api_key=Global.OPENAI_API_KEY
    )
...
def get_cached_qa_prompt(
    company: str, start_with: str, end_with: str, style: str
) -> ChatPromptTemplate:
...
async def stream_response(
    ...
    question_answer_chain = get_question_answer_chain(
        company_name, start_sentence, end_sentence, chatbot_attitude
    )
...
"""
WILL BE IMPLEMENTED IN CHAT SERVICE.
"""
